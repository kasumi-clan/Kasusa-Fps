...
h1{ font-weight:800; font-size:24px; margin:0 0 2px; letter-spacing:.2px; color:#4b4b55; }
...
.notes{ margin-top:22px; background:rgba(0,0,0,.03); border:1px solid rgba(0,0,0,.06); border-radius:14px; padding:14px 16px; }
.notes-title{ font-size:11px; font-weight:700; color:#55555f; text-transform:uppercase; letter-spacing:.5px; margin:0 0 8px; }
.notes ul{ margin:0; padding-left:18px; }
.notes li{ font-size:11.5px; color:#6b6b76; line-height:1.6; margin-bottom:4px; }
...
<div class="wrap">
  ...
  <form id="form">
    ...
    <div class="row">
      <div class="field"><label>Nâng Fps</label>
      ...
      <div class="field"><label>Nâng Độ Phân Giải</label>
    ...
    <div class="row" style="margin-bottom:20px">
      ...
      <div class="field" id="intensityField"><label>Cường độ Lag</label>
  ...
  <div class="notes">
    <p class="notes-title">📌 Lưu ý khi dùng</p>
    <ul>
      <li>Chế Độ Lag Nó Sẽ Gây Video Lag Như 15fps</li>
      <li>Chế Độ Smooth Thì Nó Sẽ Mượt</li>
      <li>1080p 2k 4k nếu bạn thấy nó vẫn như vậy là tác dụng của nó chỉ là phóng to vào máy cái khung hình 2k 4k 1080p nên nếu thấy bình thường sẽ không phải lỗi!</li>
    </ul>
  </div>
</div>
<script>
const form=document.getElementById('form'),fileInput=document.getElementById('fileInput'),fileText=document.getElementById('fileText'),submitBtn=document.getElementById('submitBtn'),progressWrap=document.getElementById('progressWrap'),progressFill=document.getElementById('progressFill'),progressLabel=document.getElementById('progressLabel'),statusText=document.getElementById('statusText'),downloadBtn=document.getElementById('downloadBtn'),errorText=document.getElementById('errorText'),previewBox=document.getElementById('previewBox'),previewVideo=document.getElementById('previewVideo'),motionSel=document.getElementById('motion'),intensityField=document.getElementById('intensityField');

function toggleIntensity(){
  intensityField.classList.toggle('hidden', motionSel.value==='smooth');
}
